...
class SystemPayCommon(PaymentAcquirerCommon):
    def setUp(self):
        super(SystemPayCommon, self).setUp()

        self.systempay = self.env['payment.acquirer'].search([('provider', '=', 'systempay')])


@common.post_install(True)
class SystemPayForm(SystemPayCommon):

    def test_00_systempay_form_render(self):
        base_url = self.env['ir.config_parameter'].get_param('web.base.url')

        # be sure not to do stupid things
        self.assertEqual(self.systempay.environment, 'test', 'test without test environment')
        self.systempay.systempay_shop_id = 'dummy'
        ...
        self.assertEqual(
            tree.get('action'),
            self.systempay.systempay_form_action_url,
            'systempay: wrong form POST URL'
        )
        ...
        for form_input in tree.input:
            ...
            self.assertEqual(
                ...
                'systempay: wrong value for input %s: received %s instead of %s' % (
                    form_input.get('name'),
                    form_input.get('value'),
                    form_values[form_input.get('name')]
                )
