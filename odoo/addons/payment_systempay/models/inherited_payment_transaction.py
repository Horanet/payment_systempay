...
class SystemPayTransaction(models.Model):
    ...
    def _systempay_form_get_tx_from_data(self, data):
        ...
        systempay_acquirer = self.env['payment.acquirer'].sudo().search([
            ('provider', '=', 'systempay')
        ])

        if signature != systempay_acquirer.systempay_generate_digital_sign(data):
            error_msg = _('SystemPay: signatures mismatch')
            _logger.info(error_msg)
            raise ValidationError(error_msg)
    ...
    def _systempay_form_get_invalid_parameters(self, data):
        ...
        if self.acquirer_id and data.get('vads_site_id') != self.acquirer_id.systempay_shop_id:
            invalid_parameters.append(
                ('vads_shop_id', data.get('vads_site_id'), self.acquirer_id.systempay_shop_id)
            )
    ...
    def _systempay_form_validate(self, data):
        ...
        :param data: data received from systempay at the end of transaction
        """
        self.ensure_one()
        ...
        if transaction_status == 'AUTHORISED':
            _logger.info('Validated SystemPay payment for transaction %s: set as done' % self.reference)
            values['state'] = 'done'
            ...
            _logger.info('Validated SystemPay payment for transaction %s: set as cancelled' % self.reference)
            values['state'] = 'cancel'
        elif transaction_status == 'INITIAL':
            _logger.info('Validated SystemPay payment for transaction %s: set as pending' % self.reference)
            values['state'] = 'pending'
        else:
            _logger.info('Validated SystemPay payment for transaction %s: set as error' % self.reference)
            _logger.info('Validated SystemPay payment for transaction %s: set as error' % self.reference)
            values['state'] = 'error'
